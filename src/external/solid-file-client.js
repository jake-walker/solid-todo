...
export function sfc($rdf, auth){
    ...
    */
    sfc.createFile = async function(url,type,content){
        var newThing = url.replace(/\/$/,'').replace(/.*\//,'')
        var parentFolder = url.replace(newThing,'').replace(/\/\/$/,'/')
        var response = await sfc.add(parentFolder,newThing,type,content)
        return response
    }
    sfc.createFolder = async function(url){
        var response = await sfc.createFile( url,"folder")
        return response
    }
    sfc.deleteFolder = async function(url) {
    ...
    sfc.updateFile = async function(url,content) {
        var del = await sfc.deleteFile( url )
        // if(!del) return false
        var add = await sfc.createFile(url,undefined,content)
        return(add)
    }
    sfc.deleteFile = sfc.deleteFolder
    sfc.readFile   = function(url){return sfc.fetch(url) }
    sfc.readFolder = async function(url){
    ...
    sfc.fetchAndParse = async function(url,contentType){
    ...
    sfc.add = async function(parentFolder,newThing,type,content) {
    ...
    sfc.fetch = async function(url,request){
    ...
    /* SESSION MANAGEMENT
    */
    sfc.checkSession = async function() { 
    ...
    sfc.login = async function(idp) {
        const session = await solid.auth.currentSession();
        if (!session)
            await solid.auth.login(idp);
        return(session.webId);
    }
    sfc.logout = async function(){
        session=''
